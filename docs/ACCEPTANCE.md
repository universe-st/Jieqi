...
### 14.9 签名 release 包（1.7.0 重打）— 撤回迷雾半透明效果
...
| 读数 | 值 |
| ---- | -- |
| 产物 | `release/jieqi-1.7.0-release.apk`，**17,881,591 B**（与 1.6.1 同字节数：代码等价，仅版本名 1.6.1→1.7.0 同长替换） |
| badging | `com.jieqi.game`、versionName **1.7.0**、versionCode **26**、minSdk 24 / target 35 / compile 35、应用名 揭棋 |
| 签名 | V2 通过，证书 SHA-256 `b0ec2bcd…89c9`（同一把密钥） |
| 改动 | **撤回 1.7.0 的迷雾半透明效果**（用户 2026-09-16 拍板：该效果把敌方视野本身暴露给玩家，泄露额外信息）。`git revert` 移除 `STEALTH_ALPHA`/`setStealth`/`dropIn targetAlpha` 与 DESIGN.md 该条规则，代码行为恢复 1.6.1；**版本号保留 1.7.0 / versionCode 26 不回退**（Android 拒绝装更低 code，若已装过含效果的 v2 仍可覆盖安装）。v2（含效果，17,881,707 B）已从坚果云撤下 |
| 门禁 | typecheck 0 错；`pnpm test` **134/134** |
| 包内核对 | `assets/www/assets/index-*.js`：**`stealth` 相关引用 0**（效果彻底出包）、circleSkin×3（1.6.1 圆形？按钮保留）、1.7.0×1 |
| 坚果云 | 已上传 `揭棋_20260916_v3.apk`（17,881,591 B，揭棋文件夹，PROPFIND `getcontentlength` 与本地字节数一致）；**`揭棋_20260916_v2.apk` 已删除** |
