...
- `禁止全局同形` 与**揭棋平台的具体判罚**（有的平台把被迫重复判负而不是判和）没有逐条对齐：
  本作在"每个合法着法都会重复"时按 `循环重复，判和` 收场，这条取的是**保守**的那一侧。
...
已上传坚果云 `揭棋/`：`揭棋_20260914_v1.apk`（1.2.1）、`_v2.apk`（1.2.2）、`_v3.apk`（1.2.3）、`_v4.apk`（1.2.4）、`_v5.apk`（1.3.0）。

> 渲染观感定论（2026-09-14）：**锐利渲染是对的**（1.2.4 恢复），当初"刺眼"是翻面揭示光过亮，不是清晰度。
> 项目自本轮起纳入 git 管理（2026-09-14 初始化，main 分支，暂无远程）。提交：`0106152`（1.2.2 基线）、
> `0e65c46`（1.2.3 还原）、`e950d46`（1.2.4 恢复锐利+柔和翻面光）、`61ec2d6`（1.3.0 BGM+提示增强）。下一版 versionCode 基线从这里读：**10**。
