...
export function displayName(board: Board, square: number): string {
  ...
  return `${piece.hidden ? '暗' : ''}${KIND_NAME[piece.color][kind]}`;
}
...
function prefixFor(board: Board, square: number, name: string): '前' | '后' | null {
  ...
  for (let sq = 0; sq < board.squares.length; sq++) {
    ...
    const otherName = `${other.hidden ? '暗' : ''}${KIND_NAME[other.color][other.hidden ? other.homeKind : other.kind]}`;
  ...
  const sorted = brothers.sort((a, b) => (piece.color === 'red' ? rankOf(a) - rankOf(b) : rankOf(b) - rankOf(a)));
...
export function toChineseNotation(board: Board, move: Move): string {
  ...
  const head = prefix ? `${prefix}${name}` : `${name}${fileNumber(piece.color, fileOf(move.from))}`;

  const dy = rankOf(move.to) - rankOf(move.from);

  if (dy === 0) {
    return `${head}平${fileNumber(piece.color, fileOf(move.to))}`;
  }

  const forward = piece.color === 'red' ? dy < 0 : dy > 0;
  const verb = forward ? '进' : '退';
  if (namesDestinationFile(kind)) {
    return `${head}${verb}${fileNumber(piece.color, fileOf(move.to))}`;
  }
